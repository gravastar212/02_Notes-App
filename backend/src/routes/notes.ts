...
import { authenticateJWT, authenticateServerSide } from '../middleware/auth.ts';
...
// GET /notes/server - Server-side notes endpoint (uses refresh token)
router.get('/server', authenticateServerSide, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// GET /notes - List user's notes (client-side with access token)
